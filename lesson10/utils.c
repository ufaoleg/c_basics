...
void generate_data_array(measure_t *arr_measures, int arr_size)
{
    ...
    {   
        // *arr[i] = "%d;%d;%d;%.1f\n", generate_day_i(i), generate_month_i(i), generate_year_i(i), generate_temp_f(i);
        // // printf("%d;%d;%d;%.1f\n", generate_day_i(i), generate_month_i(i), generate_year_i(i), generate_temp_f(i));
        // strcpy(arr_measures[i].day, generate_day_i(i));
        // strcpy(arr_measures[i].month, generate_month_i(i));
        // strcpy(arr_measures[i].year, generate_year_i(i));
        // strcpy(arr_measures[i].temp, generate_temp_f(i));
